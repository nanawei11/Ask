...
def plot_amp(rois, segs, links, bgs, beds, fig_out = 'test.pdf',
    ...
    # set figure height
    fig_height = 4.3 + segs_height + beds_height
    ...
    plt.subplots_adjust(left = 0.11, right = 0.92, bottom = 0.19, top = 0.92) 
    # plot tracks
...
class Bed:
    ...
    def print(self):
        ...
        if self.type == 'bed4':
            print(self.chrom, self.start, self.end, self.name)
        if self.type == 'bed6':
...
class Bgtrack:
    ...
    def convert_bgs(bgs, rois, gap):
        ...
        # convert
        for roi in rois:
            current_end = current_start + roi[2] - roi[1]  # gap+len(roi)
            ...
            bgs_cvt.append(pd.DataFrame([[roi[0], current_end, 0]],  # char7   current_start+len(rois) 0
                columns = ['Chrom', 'Coord', 'CN']))
            bgs_cvt.append(pd.DataFrame([[roi[0], current_start, 0]],   # # char7   current_start 0
                columns = ['Chrom', 'Coord', 'CN']))
...
class Arctrack:
    ...
    def plot(self):
        ...
        for lk in links:
            ...
            # if the links is visiual [virtual, 0, virtual, 1, 0]
            isvirtual = lk.chrom1 == 'virtual' and lk.chrom2 == 'virtual'
            if isvirtual:
                color='white'
            # plot arc
            ax.add_patch(Arc((center, 1), width, height, 0, 0, 180,    # angle=0, theta1=0, theta2=180
                              color = color, linewidth = linewidth))

            if not isvirtual:
                # plot arc score
                ax.text(center, height / 2, lk.score,    # x pos, y pos, show text
                        horizontalalignment = 'center',  # 水平居中
                        verticalalignment = 'bottom',    # 下面与y齐，相当于在线上面
                        fontsize = fontsize)
...
class Genetrack:
    ...
    #--------------------------------------------------------------------------#
    def __init__(self, ax, roi, beds, char_len, way = 1):
        self.ax = ax
        self.roi = roi
        self.beds = beds
        self.char_len = char_len
    ...
    def plot(self):
        ...
        for rg in rois:
            ...
            x_convert_major.append((cvt_start + cvt_end) / 2)
            x_convert.append(cvt_end)
            x_actual.append(f'{rg[1]:,}')
            x_actual_major.append('\n' + rg[0])
            x_actual.append(f'{rg[2]:,}')
            cvt_start = cvt_end + 1 + gap
            # ax.text((rg[1] + rg[2]) / 2, 1, rg[0])
        ax.xaxis.set_major_locator(ticker.FixedLocator(x_convert_major))
        ax.xaxis.set_major_formatter(ticker.FixedFormatter(x_actual_major))
        ax.xaxis.set_minor_locator(ticker.FixedLocator(x_convert)) #  设置次要坐标点，起始点
        ax.xaxis.set_minor_formatter(ticker.FixedFormatter(x_actual)) #  设置次要坐标点， 终止点
        ...
        for idx, bed in enumerate(beds):
            if (max(ys) == self.max_row):
                ypos = ys[idx] - 1
                if ypos != -1:
                    self.draw_gene(bed, ypos)
            else:
    ...
    def get_gene_ypos(self):
        ...
        ys = []
        ends = [0] * (max_row + 1) # end position of each row
        for bed in beds:
            offset = char_len * (len(bed.name))
            for idx, end in enumerate(ends):
                if (end == 0 or bed.start > end + offset +  self.arrow_length * 0.6):
                    # bed.ypos = idx
                    ys.append(idx)
                    ends[idx] = bed.end
                    break
...
class Bedtrack(Genetrack):
    ...
    def draw_region(self, bed, ypos, color = None):
        ...
        # draw bed region
        if (bed.strand not in ['+', '-']):  # bed.strand
            ax.add_patch(Rectangle((bed.start, ypos),  # left bottom pos
                        bed.end - bed.start, height,   # width height
                        edgecolor = edgecolor,
                        facecolor = facecolor,
                        linewidth = linewidth))
        else:
