...
class GRange:
    ...
    def create_gr(self, genomic_range, type):
        ...
        elif (type == 'bpdataframe'):
            return self.create_gr_from_bpdf(genomic_range)
        elif (type == 'list'):
            return self.create_gr_from_list(genomic_range)
        else:
    ...
    def create_gr_from_bpdf(df):
        ...
        for word in df.itertuples():
            ...
            try:
                ...
                ext = (word[0],)
            op.append(((chrom1, range(start1, end1)), 
                       (chrom2, range(start2, end2)), 
                        ext))
        return op
    
    @staticmethod
    def create_gr_from_list(genomic_range):
        op = []
        for line in genomic_range:
            chrom = line[0]
            # print(line)
            start = int(line[1])
            end = int(line[2])
    ...
    #--------------------------------------------------------------------------#
    def intersect(self, b, a_extend = 0, b_extend = 0, invert = False, mode=1):
        """
        ...
        if mode == 1:
            ...
            for x in a:
                if invert:
                    ...
                    tf = False
                for y in b:
                    if (self.interval_overlap(x[0], y, a_extend, b_extend)) or (self.interval_overlap(x[1], y, a_extend, b_extend)):
                        if invert:
                            tf = False
                        else:
                            tf = True
    ...
    def map_senchancer(df, bedse):
        ...
        """
        gr1 = GRange(df, 'dataframe_hasend')
        gr2 = GRange(bedse, 'bedfile')
        gr = gr1.gmap(gr2, multi_hit = True)
        return df.assign(SE = ['; '.join(set([i[0] for i in row])) for row in gr])

    #--------------------------------------------------------------------------#
    def gr_to_df(self, n=5):
        return self.gr[0:n]
